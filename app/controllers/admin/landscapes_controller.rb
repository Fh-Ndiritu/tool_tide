class Admin::LandscapesController < Admin::BaseController
  def index
    if params[:day].present? && params[:day].to_i.positive?
      @landscapes = Landscape.includes(:user).where(created_at: params[:day].to_i.days.ago.beginning_of_day..params[:day].to_i.days.ago.end_of_day).order(created_at: :desc)
    else
