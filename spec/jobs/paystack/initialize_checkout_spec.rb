...
RSpec.describe Paystack::InitializeCheckout do
  ...
  describe '#validate_data' do
    ...
    context 'when status is false' do
      it 'returns Failure' do
        expect(subject.send(:validate_data, invalid_response.with_indifferent_access)).to eq(Failure('PayStack Status is not true'))
      end
    end
    ...
    context 'when access_code or authorization_url is blank' do
      ...
      it 'returns Failure' do
        expect(subject.send(:validate_data, valid_response.with_indifferent_access)).to eq(Failure('Paystack access_code or authorization_url is blank'))
      end
    end
    ...
    context 'when reference key has changed' do
      ...
      it 'returns Failure' do
        expect(subject.send(:validate_data, valid_response.with_indifferent_access)).to eq(Failure('Paystack reference key has changed'))
      end
    end
  end
